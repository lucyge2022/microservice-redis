...
impl Service for Microservice {
    ...
    fn call(&self, request: Request) -> Self::Future {
        debug!("{:?}", request);
        let redis_connection = self.create_redis_conn();
        info!("LUCY: got redis conn");
        ...
        match (request.method(), request.path()) {
            (&Post, "/") => {
                let future = request
                    ...
                    .and_then(move |new_message| process_new_message(new_message))
            ...
            (&Get, "/") => {
                ...
                let response = match time_range {
                    Ok(time_range) => futures::future::ok(
                        Response::new().with_status(StatusCode::Ok)),
                        // make_get_response(query_db(time_range, &db_connection)),
...
impl Microservice {
    fn create_redis_conn(&self) -> Result<redis::Connection, RedisError> {
        let client = redis::Client::open("redis://127.0.0.1/")?;
